  ...
  return (
    <>
      ...
      <p class="mt-3">
        ...
        <a class={linkStyles} href="https://deno.com/deploy">Deno Deploy</a>
        {" "}
        means serving requests from locations around the globe, from a location
        closest to the user. Ideally, the database would be located close to the
        server so that latency between the server and database is minimised for
        all the database calls made from the server. Therefore an ideal database
        for Deno Deploy would be one which was globally distributed in multiple
        regions to decrease latency and thus provide a better user experience.
        In this post/web application, we'll experiment with the following
        serverless databases which all offer multi-region replication and work
        with Deno Deploy:
      </p>
      ...
      <p class="mt-3">
        ...
        the open source{" "}
        <a class={linkStyles} href="https://www.foundationdb.org/">
          FoundationDB
        </a>{" "}
        ...
        </a>, the strongest level of consistency possible.
      </p>
      ...
      <p class="mt-3">
        <a class={linkStyles} href="https://aws.amazon.com/dynamodb/">
          DynamoDB
        </a>{" "}
        is a serverless NoSQL key-value database from AWS. For access, it
        supports a custom{" "}
        <a
          class={linkStyles}
        ...
        <a
          class={linkStyles}
          href="https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ql-reference.html"
        >
          PartiQL
        </a>{" "}
        (a SQL-compatible query language). DynamoDB tables, by default, are
        deployed to a single region. To achieve a multi-region setup, you can
        enable{" "}
        <a
          class={linkStyles}
          href="https://aws.amazon.com/dynamodb/global-tables/"
        >
          global tables
        </a>{" "}
        which creates identical tables in additional regions (in as many regions
        as you like). Only a single region in N. Virginia was configured for
        this experiment.
      </p>
      <p class="mt-3">
        Strong consistency is offered within a single region only but not for
        secondary indexes. Global tables are eventually consistent with each
        other. DynamoDB also supports ACID transactions, but only within a
        single region. Transactions are not ACID compliant when replicating to
        other regions. DynamoDB also integrates extremely well with other AWS
        services as well as offering a host of enterprise features such as
        encryption at rest and a multitude of back and restore options.
      </p>
      ...
      <p class="mt-3">
        <a class={linkStyles} href="https://fauna.com/">Fauna</a>{" "}
        is a serverless NoSQL{" "}
        <a
          class={linkStyles}
          href="https://fauna.com/blog/what-is-a-document-relational-database"
        >
          document-relational
        </a>{" "}
        database offering two query languages,{" "}
        <a
          class={linkStyles}
        ...
        >
          GraphQL
        </a>. During database creation, you must choose between two{" "}
        <a
          class={linkStyles}
          href="https://docs.fauna.com/fauna/current/learn/understanding/region_groups"
        >
          region groups
        </a>{" "}
        to host your database: US or Europe. Data is replicated to 3 geographic
        regions within the region group (i.e. US or Europe), but never outside
        that region group. For this experiment the US region group was chosen.
        True global replication is possible (e.g. hosting in US, Europe and
        Aisa) but only for enterprise customers through their Virtual Private
        Fauna offering.
      </p>
      <p class="mt-3">
        Fauna offers strong consistency for all reads and writes as well as
        supporting ACID transactions. Both Deno KV and Fauna provide the
        strongest consistency model of the databases in the experiment. Despite
        being a NoSQL database, a strength of Fauna is its relational database
        capabilities such as normalisation, joins and indexes.
      </p>
      ...
      <p class="mt-3">
        <a class={linkStyles} href="https://planetscale.com/">PlanetScale</a>
        {" "}
        offers a serverless MySQL-compatible database built on top of{" "}
        <a class={linkStyles} href="https://vitess.io/">Vitess</a>{" "}
        and is the only true relational database of the experiment. It offers 11
        AWS regions with 4 GCP regions. For this experiment, a primary region of
        N. Virginia was selected. Additional replica regions are only available
        on paid plans (starting at $29/month) and are therefore not included in
        this experiment. Being a relational database, access to the data is done
        via normal SQL queries. A key offering is the ability to change the
        schema in isolated branches, similar to a git workflow. All data is
        automatically encrypted at rest. Like MySQL, it only offers eventual
        consistency.
      </p>
      <p class="mt-3">
        Though serverless, PlanetScale isn't necessarily hands off as when your
        database becomes large you may need to intervene and apply{" "}
        <a
          class={linkStyles}
          href="https://www.digitalocean.com/community/tutorials/understanding-database-sharding"
        >
          sharding
        </a>, which can be complicated.
      </p>
      ...
      <p class="mt-3">
        <a class={linkStyles} href="https://upstash.com/">Upstash</a>{" "}
        takes the well known in-memory{" "}
        <a class={linkStyles} href="https://redis.io/">Redis database</a>{" "}
        a step further with a durable Redis compatible global database. Access
        is via Redis APIs. It offers per request pricing and which scales to
        zero price if not used (storage still accrues cost). Upstash Redis only
        offers eventual consistency. For this experiment, a primary region of N.
        Virginia was configured with replicas setup in all available replica
        regions (California, Oregon, Frankfurt, Ireland, Singapore, Sydney and
        Sao Paulo). N.b. this would be a potentially expensive setup for a high
        volume application.
      </p>
        ...
        <div class="mt-5 overflow-x-auto border-1 rounded-md">
          <table class="min-w-full text-left text-sm font-light bg-[#202020]">
            <thead class="border-b font-medium">
              <tr>
                <th class="sticky left-0 z-100 px-6 py-3 bg-[#202c2c]">
                  Database
                </th>
                <th class="min-w-[100px] bg-[#202c2c]">Type</th>
                <th class="min-w-[100px] bg-[#202c2c]">
                  Regions configured in experiment<br />(* primary)
                </th>
            ...
            <tbody>
              ...
              <tr class="border-b">
                ...
                <td>N. Virginia*</td>
                <td>
                  Strong w/in single region (except for secondary indexes),
                  eventual outwith (and optional within).
                </td>
                <td>PartiQL or custom CRUD API</td>
                <td>
                  At best, serializable ACID transactions in single region only.
                  Some operations are read-committed or not serializable
                </td>
              </tr>
              <tr class="border-b">
                <td class="sticky left-0 z-10 whitespace-nowrap px-6 py-3 font-medium bg-[#202020]">
              ...
              <tr class="border-b">
                ...
                <td>Key/Value</td>
                <td>
                  N. Virginia*, California, Oregon, Frankfurt, Ireland,
